...
RuntimeEntityRegistry::RuntimeEntityRegistry(std::uint64_t scene_generation) : scene_generation_{scene_generation} {
    if (scene_generation_ == 0U) {
        throw std::invalid_argument{"runtime entity registry requires a non-zero scene generation"};
    }
}
...
RuntimeEntityHandle RuntimeEntityRegistry::create() {
    ...
    return {
        ...
        .index = static_cast<std::uint32_t>(slots_.size() - 1U),
        .generation = 1U,
    };
}
...
SceneInstance::SceneInstance(std::uint64_t scene_generation, ContextOrigin origin,
    ...
    : scene_generation_{scene_generation}, origin_{origin}, readiness_{required_content},
      physics_world_{std::move(physics_world)}, entities_{scene_generation} {
    if (scene_generation_ == 0U) {
...
void SceneInstance::activate() {
    if (state_ != SceneState::Ready) {
        throw std::logic_error{"scene can activate only after required content is ready"};
    }
    if (!physics_world_) {
...
physics::PhysicsWorld& SceneInstance::physics_world() {
    if (!physics_world_ ||
        (state_ != SceneState::Ready && state_ != SceneState::Active && state_ != SceneState::Quiescing)) {
...
void ActiveLocalContext::activate(ActiveLocalContextDescriptor descriptor, std::unique_ptr<SceneInstance> scene) {
    if (!scene) {
        throw std::invalid_argument{"active local context requires a scene"};
    }
    ...
    if (descriptor.scene_generation != scene->scene_generation() ||
        descriptor.origin_generation != scene->origin().generation) {
    ...
    if (scene->state() != SceneState::Ready) {
        throw std::logic_error{"candidate scene must be ready before activation"};
    }

    if (scene_) {
        if (scene_->state() == SceneState::Active) {
            scene_->quiesce();
        }
        scene_->deactivate();
        scene_.reset();
...
SceneInstance& ActiveLocalContext::scene() {
    if (!has_active_scene()) {
        throw std::logic_error{"no active local scene"};
    }
    return *scene_;
}
