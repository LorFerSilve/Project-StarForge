...
[[nodiscard]] std::uint32_t load_be32(std::span<const std::byte> bytes, std::size_t offset) noexcept {
...
ContentFingerprint sha256(std::span<const std::byte> bytes) {
    ...
    for (std::size_t block_offset = 0; block_offset < padded.size(); block_offset += 64U) {
        const auto block = std::span<const std::byte>{padded}.subspan(block_offset, 64U);
        ...
        for (std::size_t index = 0; index < schedule.size(); ++index) {
            ...
            const auto temp1 = h + sum1 + choose + round_constants[index] + schedule[index];
